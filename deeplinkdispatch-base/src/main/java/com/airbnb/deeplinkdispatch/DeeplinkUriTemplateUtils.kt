...
fun String.placeholders(): List<String>? {
    return placeholderRegex.findAll(this).map { it.groupValues[0] }.toList().takeUnless { it.isEmpty() }
}
...
internal fun String.placeholderValues(): List<String> {
    return allowedPlaceholderRegex.find(this)?.let { matchResult ->
...
internal fun String.orderPlaceholderValues(): String {
    return allowedPlaceholderRegex.replace(this) { matchResult ->
        matchResult.value.split(MatchIndex.ALLOWED_VALUES_SEPARATOR).sorted()
...
fun String.allPossibleValues(): List<String> {
    return placeholders()?.firstOrNull()?.let { firstPlaceholder ->
