...
public class DeepLinkProcessor extends AbstractProcessor {
  ...
  private void generateDeepLinkLoader(List<DeepLinkAnnotatedElement> elements) throws IOException {
    ...
    MethodSpec parseMethod = MethodSpec.methodBuilder("parseUri")
        ...
        .returns(DeepLinkMatch.class)
        .beginControlFlow("for (DeepLinkEntry entry : registry)")
        .addStatement("final Optional<DeepLinkMatch> match = entry.matches(uri)")
        .beginControlFlow("if (match.isPresent())")
        .addStatement("return match.get()")
        .endControlFlow()
        .endControlFlow()
  ...
  private void generateDeepLinkActivity() throws IOException {
    ...
    MethodSpec onCreateMethod = MethodSpec.methodBuilder("onCreate")
        ...
        .addStatement("DeepLinkMatch deepLinkMatch = loader.parseUri(uriString)")
        .beginControlFlow("if (deepLinkMatch != null)")
        .addStatement("DeepLinkEntry entry = deepLinkMatch.getEntry()")
        .addStatement("DeepLinkUri deepLinkUri = deepLinkMatch.getUri()")
        .addStatement("$T<String, String> parameterMap = deepLinkMatch.getParameters()", Map.class)
