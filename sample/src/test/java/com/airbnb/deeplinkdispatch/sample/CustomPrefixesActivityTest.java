...
public class CustomPrefixesActivityTest {
    @Test
    public void testAppDeepLinkIntent() {
        ...
        assertThat(launchedIntent.getStringExtra(DeepLink.URI), equalTo("app://airbnb/view_users?param=foo"));
        assertThat(launchedIntent.getExtras().getString("param"), equalTo("foo"));
    }

    @Test
    public void testWebDeepLinkIntent() {
    ...
    @Test
    public void testWebDeepLinkIntentWithId() {
        List<String> cases = ImmutableList.of("http://airbnb.com/user/123", "https://airbnb.com/user/123");
        for (String uri : cases) {
            Intent launchedIntent = getLaunchedIntent(uri);
            assertThat(launchedIntent.getComponent(),
                    equalTo(new ComponentName(RuntimeEnvironment.application, CustomPrefixesActivity.class)));
